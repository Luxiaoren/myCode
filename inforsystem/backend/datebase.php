...
function addcount(){
    ...
    $pass=$_POST['pass'];
    $count=$_POST['count'];
    $sql="insert into logcount(username,password,counttype)values ('$user','$pass','$count')";
    $res=mysql_query($sql);
    if(!$res){
        echo "添加失败";
        die();
...
function delcount(){
    $id=$_POST['delid'];
    $sql="delete from logcount where id=$id";
    $res=mysql_query($sql);
    if(!$res){
        echo "删除失败";
        die();
...
function index()
{
    ...
    $sql = "select *from logcount where username='$username'";
    $res = mysql_query($sql);
...
//    echo $user.$uptime.$filename;
    $sql = "insert into file(filename,time,peoname)value('$filename','$uptime','$user')";
    $res = mysql_query($sql);
    if (!$res) {
        echo "上传失败";
        die();
